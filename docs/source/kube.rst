Scaling [elastalink-3f10-ml] With Kubernetes
===========================
...
Based on your TML solution [elastalink-3f10-ml] - if you want to scale your application with Kubernetes - you will need to apply the following YAML files.

.. list-table::

   * - **YML File**
     - **Description**
   * - :ref:`elastalink-3f10-ml.yml`
   ...
   * - :ref:`qdrant.yml`
     - This is the Qdrant deployment YAML.
       ...
       applied if using Step 9 DAG.
   * - :ref:`nginx-ingress-elastalink-3f10-ml.yml`
     - If you are scaling your TML solution you must

       apply the nginx-ingresselastalink-3f10-ml.yml; this yaml is 
...
.. code-block:: YAML
   ...
   kubectl apply -f kafka.yml -f secrets.yml -f mysql-storage.yml -f mysql-db-deployment.yml -f elastalink-3f10-ml.yml

elastalink-3f10-ml.yml
------------------------

.. important::
   Copy and Paste this YAML file: elastalink-3f10-ml.yml - and save it locally.
...
.. code-block:: YAML
   ...
   ################# elastalink-3f10-ml.yml
     ...
     kind: Deployment
     metadata:
       name: elastalink-3f10-ml
     spec:
       selector:
         matchLabels:
           app: elastalink-3f10-ml
       ...
       template:
         ...
         spec:
           containers:
           - name: elastalink-3f10-ml
             image: maadsdocker/elastalink-3f10-ml-amd64:latest
             volumeMounts:
             - name: dockerpath
             ...
             - name: TSS
               value: '0'
             - name: SOLUTIONNAME
               value: 'elastalink-3f10-ml'
             - name: SOLUTIONDAG
               value: 'solution_preprocessing_ml_dag-elastalink-3f10'
             - name: GITUSERNAME
               value: 'smaurice101'
             - name: GITREPOURL
             ...
             - name: step5processlogic # STEP 5 processlogic field can be adjusted here.  
               value: 'classification_name=failure_prob:CPULoad_preprocessed_Avg=90,n'                                                
             - name: step5independentvariables # STEP 5 independent variables can be adjusted here.  
               value: 'CPULoad_preprocessed_Avg'                                                                              
   ...
   ---
     ...
     kind: Service
     metadata:
       name: elastalink-3f10-ml-visualization-service
       labels:
         app: elastalink-3f10-ml-visualization-service
     spec:
       type: ClusterIP
       ports:
...
.. code-block::
   ...
   kubectl port-forward deployment/elastalink-3f10-ml 6060:6060

After you forward the ports then copy/paste the viusalization URL below and run your dashboard.

.. code-block::

   http://localhost:6060/iot-failure-machinelearning-elastalink.html?topic=iot-preprocess,iot-ml-prediction-results-output&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
...
.. important::
   ...
   **STEP 4:  Apply nginx-ingress-elastalink-3f10-ml.yml to your kubernetes cluster.  First you need to save it locally then apply it:**

nginx-ingress-elastalink-3f10-ml.yml
-------------
   ...
   .. code-block::
      ...
      kubectl apply -f nginx-ingress-elastalink-3f10-ml.yml
...
.. code-block::
   ...
   http://tml.tss/viz/iot-failure-machinelearning-elastalink.html?topic=iot-preprocess,iot-ml-prediction-results-output&offset=-1&groupid=&rollbackoffset=400&topictype=prediction&append=0&secure=1
